 ...
 *
 * Read from the package manifest instead of restated here. The two used to be
 * duplicated, with a comment claiming a test kept them in sync — there was no
 * such test, so the reported version could have gone stale the first time the
 * manifest moved. Deriving it makes that class of drift impossible; the fallback
 * keeps a stripped checkout loadable rather than failing at import.
 */
...
Options
  ...
  --workspace <dir>   the session workspace to include (default: $DSH_WORKSPACE, or the
                      path recorded by "init"). Without one, only the harness home is synced.
...
What is synced
  The harness home (settings, profiles, skills, presets) and the workspace you
  point at with --workspace: its session configuration (.dsh/), skills, and the
  plain notes the allowlist names. Git repositories inside the workspace are
  recorded as clone references, not copied. Research data, build output and
  anything holding a credential are never included; "push" additionally screens
  every file and refuses to upload a value that looks like a secret.
...
const VALUE_FLAGS = new Set(['root', 'data-root', 'profile', 'device', 'url', 'user', 'repo', 'branch', 'to', 'workspace']);

/** Options that are booleans. */
const BOOLEAN_FLAGS = new Set(['force', 'dry-run', 'no-install', 'list', 'json', 'no-color', 'help', 'version', 'verbose']);
...
export async function dispatch(argv, overrides = {}) {
	...
	// The CLI accepts the conventional kebab-case spelling (`--dry-run`), while
	// commands read camelCase (`flags.dryRun`). Bridge the two once, here, so a
	// flag can never half-work: `--dry-run` used to parse fine and then do nothing,
	// because nothing ever populated `flags.dryRun` — a "dry run" that pushed.
	for (const [name, value] of Object.entries(parsed.flags)) {
		const camel = name.replace(/-([a-z])/g, (_match, letter) => letter.toUpperCase());
		if (camel !== name && parsed.flags[camel] === undefined) parsed.flags[camel] = value;
	}
	...
	try {
		ctx = loadContext({
			...
			workspace: (flags.workspace ?? overrides.workspace),
		...
		if (process.env.HARNESS_SYNC_DEBUG === '1') {
			ui.write(`DEBUG flags.workspace=${flags.workspace} overrides.workspace=${overrides.workspace} ctx.workspace=${ctx.workspace}`);
		}
	} catch (error) {
