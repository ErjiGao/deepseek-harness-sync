 ...
 *
 * @param {{dshHome: string, profile: string, backupDir: string, device: string, version?: number, reason: string}} options - inputs.
 * @returns {{path: string, fileCount: number}} the written backup.
 */
export function createBackup(options) {
	const { dshHome, profile, backupDir, device, version = 0, reason } = options;
	const { files, absent } = collect({ dshHome, profile });
	const envelope = buildSnapshot({ version, device, profile, files, absent, reason });
	mkdirSync(backupDir, { recursive: true });
	const path = freeName(backupDir);
	writeFileSync(path, stableStringify(envelope), { encoding: 'utf8', mode: 0o600 });
	return { path, fileCount: Object.keys(files).length };
}
