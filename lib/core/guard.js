 ...
 * So do the ordinary ways a program reads one, which a workspace full of research
 * scripts contains constantly:
 *
 *     process.env.OPENAI_API_KEY          (Node)
 *     os.environ["OPENAI_API_KEY"]        (Python)
 *     os.environ.get("OPENAI_API_KEY")    (Python)
 *     os.getenv("OPENAI_API_KEY")         (Python)
 *     ENV["OPENAI_API_KEY"]               (Ruby)
 *     getenv("OPENAI_API_KEY")            (C / PHP)
 *
 * Missing these made the screen block legitimate code, which matters more than it
 * looks: a credential screen that cries wolf on `process.env.X` is a screen the
 * user turns off, and a disabled screen catches nothing.
 *
 * @param {string} value - the bare value.
...
function looksLikeReference(value) {
	...
	// An environment read, in the spellings that actually occur.
	if (/^(process\.env|os\.environ|os\.getenv|ENV|getenv)\b/i.test(value)) return true;
	if (/^(process\.env|os\.environ|os\.getenv|ENV|getenv)\s*[.[(]/i.test(value)) return true;
	return false;
}
