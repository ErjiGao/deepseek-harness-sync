...
import { assertManaged, splitSnapshotPath } from './manifest.js';
...
/**
 * Validate one snapshot entry and resolve it to a native path.
 *
 * The validation is chosen by the entry's own prefix, so a workspace path can
 * never be accepted on the strength of the home's whitelist or the reverse. When
 * an entry names the workspace but no workspace root is configured, that is an
 * error rather than a silent skip: applying the rest would leave the machine
 * quietly incomplete.
 *
 * @param {{dshHome: string, workspace?: string, profile: string, rel: string}} options - the roots and the entry.
 * @returns {string} the native absolute destination.
 * @throws {Error} when the entry is unsafe, outside its whitelist, or unroutable.
 */
export function resolveEntry(options) {
	const { dshHome, workspace, profile, rel } = options;
	const split = splitSnapshotPath(rel);
	if (split.target === 'workspace') {
		if (typeof workspace !== 'string' || workspace === '') {
			throw new Error(`refusing ${rel}: it belongs to a workspace, but no workspace root is configured on this machine`);
		}
 ...
 *
 * @param {{dshHome: string, profile: string, workspace?: string, envelope: object}} options - inputs.
 * @returns {{written: string[], unchanged: string[], deleted: string[], notes: string[]}} the plan.
 */
export function planApply(options) {
	const { dshHome, profile, workspace, envelope } = options;
	/** @type {string[]} */
	const written = [];
	/** @type {string[]} */
	const unchanged = [];
	...
	for (const [rel, text] of Object.entries(envelope.files)) {
		let abs;
		try {
			abs = resolveEntry({ dshHome, workspace, profile, rel });
		} catch (error) {
			notes.push(error.message);
			continue;
		}
		if (readIfFile(abs) === text) unchanged.push(rel);
 ...
 *
 * @param {{dshHome: string, profile: string, workspace?: string, envelope: object, dryRun?: boolean}} options - inputs.
...
export function applySnapshot(options) {
	const { dshHome, profile, workspace, envelope, dryRun = false } = options;
	const plan = planApply({ dshHome, profile, workspace, envelope });
	...
	for (const rel of plan.written) {
		writeAtomic(resolveEntry({ dshHome, workspace, profile, rel }), envelope.files[rel]);
	}
	for (const rel of plan.deleted) {
		rmSync(resolveEntry({ dshHome, workspace, profile, rel }), { force: true });
	}
	return plan;
}
