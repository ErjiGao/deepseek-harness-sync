...
export function apply(ctx, rawConfig) {
	...
	ctx.tools.register(
		commandTool({
			...
			command: 'push',
			description:
				'Upload this machine\'s DeepSeek Harness configuration to the private configuration repository. Refuses to run if the files contain anything that looks like a credential, and refuses to overwrite a repository that advanced since the last sync unless force is set.',
	...
	ctx.tools.register(
		commandTool({
			...
			command: 'pull',
			description:
				'Download the configuration from the private repository and apply it to this machine. Always writes a local backup first, so the change can be undone with harness_sync_rollback.',
			parameters: {
				type: 'object',
				properties: { dryRun: { type: 'boolean', description: 'Report what would change and apply nothing.' } },
				additionalProperties: false,
			},
			boolean: ['dryRun'],
		}),
