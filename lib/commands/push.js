...
export async function run(ctx, options = {}) {
	...
	// 2b. Never drop a layer this run simply could not see.
	//
	// A push with no workspace configured publishes a snapshot with no `workspace/`
	// entries, which *removes* the workspace layer from the repository — not because
	// anyone deleted it, but because this process was never told where the workspace
	// is. The version check above cannot catch that: the remote version advanced only
	// through this plugin's own pushes, so nothing ever looks diverged. Refuse, and
	// say exactly how to include it (or how to drop it on purpose).
	const countUnder = (files, prefix) => Object.keys(files).filter((rel) => rel.startsWith(prefix)).length;
	const remoteWorkspace = countUnder(situation.remoteFiles, 'workspace/');
	const localWorkspace = countUnder(situation.localFiles, 'workspace/');
	if (remoteWorkspace > 0 && localWorkspace === 0 && !force) {
		ui.fail(`the repository holds ${remoteWorkspace} workspace file(s), but this run collected none.`);
		ui.write('      Publishing now would delete the workspace layer from the snapshot — not because it is');
		ui.write('      gone, but because no workspace was configured for this run.');
		ui.write('');
		ui.write('      Include it:            harness-sync push --workspace <path>');
		ui.write('      Drop it on purpose:    harness-sync push --force');
		return 2;
	}
