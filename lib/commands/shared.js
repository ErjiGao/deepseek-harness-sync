...
/**
 * Markers fencing this plugin's own block inside the configuration repository's
 * `.gitignore`.
 *
 * The fence exists so the block can be *replaced* rather than only appended: a
 * rule change (dropping `sessions/` once session logs became a synced layer) must
 * reach a repository that already carries an older block, while any rules another
 * tool wrote outside the fence stay exactly where they are.
 */
export const GITIGNORE_BEGIN = '# --- deepseek-harness-sync secrets (do not remove this line) ---';
export const GITIGNORE_END = '# --- end deepseek-harness-sync ---';

/** Contents written to `.gitignore` inside the configuration repository, as a second line of defence. */
export const CONFIG_REPO_GITIGNORE = `${GITIGNORE_BEGIN}
# Nothing in this plugin ever stages these, and this fenced block makes sure a
# stray manual \`git add -A\` cannot publish them either.
#
# \`sessions/\` is deliberately NOT listed: session logs are a synced layer, stored
# as real files at this path (see core/sessions.js). Ignoring them here would make
# every push report sessions it had not actually committed.
.credentials.yaml
*.token
.env
*.sqlite
...
/**
 * Fold this plugin's fenced block into an existing `.gitignore`.
 *
 * Whatever sits outside the fence belongs to someone else and is left untouched;
 * whatever sits inside it is ours and is replaced wholesale, which is what lets a
 * rule change propagate to a repository that already has an older block.
 *
 * @param {string} current - the existing document.
 * @returns {string} the merged document.
 */
export function mergeGitignore(current) {
	const begin = current.indexOf(GITIGNORE_BEGIN);
	if (begin === -1) {
		const head = current === '' || current.endsWith('\n') ? current : `${current}\n`;
		return `${head}\n${CONFIG_REPO_GITIGNORE}`;
	}
	const end = current.indexOf(GITIGNORE_END, begin);
	if (end === -1) {
		// A truncated block, from an older build or a hand edit: take the rest too.
		return `${current.slice(0, begin)}${CONFIG_REPO_GITIGNORE}`;
	}
	const after = current.slice(end + GITIGNORE_END.length).replace(/^\r?\n/, '');
	return `${current.slice(0, begin)}${CONFIG_REPO_GITIGNORE}${after}`;
}
...
export function makeEnvelope(ctx, local, version, reason) {
	return buildSnapshot({
		...
		files: local.files,
		absent: local.absent,
		// Repository references and the session index both travel in `preferences`,
		// which older builds already pass through untouched, so a snapshot carrying
		// them still applies on a machine running an earlier version of this plugin.
		// The session index holds paths and hashes only; the logs themselves live as
		// real files in the repository (see core/sessions.js).
		preferences: {
			...repositoryPreferences(local.repositories ?? []),
			...((local.sessions ?? []).length > 0 ? { sessions: local.sessions } : {}),
		},
