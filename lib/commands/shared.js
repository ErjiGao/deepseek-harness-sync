...
/**
 * Read this machine's configuration as a file map.
 *
 * @param {object} ctx - the context.
 * @returns {{files: Record<string, string>, absent: string[], notes: string[], machine: {rel: string, line: number, spec: string}[]}} the local state.
 */
export function readLocal(ctx) {
	const { files, absent, notes } = collect({ dshHome: ctx.paths.dshHome, profile: ctx.profile });
	return { files, absent, notes, machine: detectMachineDependencies(files, ctx.profile) };
}
 ...
 *
 * @param {object} ctx - the context.
 * @param {{files: Record<string, string>, absent: string[]}} local - the local files.
